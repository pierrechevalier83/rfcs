...
1. Every fixed interval, each group computes a hash of all the checkpoints generated during that
period. This hash will then be exchanged with group's sibling group to compute an one level up hash
(say group 000 exchanging with group 001 to compute hash for 00x).
